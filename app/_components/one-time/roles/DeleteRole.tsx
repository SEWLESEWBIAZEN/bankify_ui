'use client'
import { deleteAppRole } from '@/app/_lib/actions/auth'
...
const DeleteRole = ({ id }: { id: number }) => {
    const initialState: DeleteState = { success: null, submitError: null }
    
    const [state, deleteAction, isPending] = useActionState(deleteAppRole, initialState);

    useEffect(() => {
        if (state.success) {
            toast.success(state.success ?? "Role deleted!");
        ...
        <Dialog>
            ...
            <DialogContent>
                <form action={deleteAction.bind(null,id)}>
                    ...
                    <DialogDescription>Please be aware of the result of deleting this role.</DialogDescription>
